...
| `(tabs)/index.tsx` | Home screen with time-of-day greeting, Analyze button |
| `(tabs)/history.tsx` | Meal history list with swipe-to-delete, date grouping, calorie summary |
| `camera.tsx` | Camera/gallery capture UI |
| `results.tsx` | Analysis results modal (slides from bottom); animated calorie counter |
...
| Service | Purpose |
|---|---|
| `api.ts` | API client (fetch, AbortController timeouts, 20 MB image validation) |
| `storage.ts` | AsyncStorage meal history (200-meal cap, oldest auto-pruned) |

#### Utilities (`src/utils/`)
| Utility | Purpose |
|---|---|
| `helpers.ts` | Formatting (calories, grams, confidence, dates); extraction functions |
...
| `analyzing-overlay.tsx` | 4-step cycling progress indicator with text |
...
## Mobile UX Components (Phase 1–4)
...
- **AnalyzingOverlay** — 4-step cycling indicator with text
...
| File | Lines | Status |
|---|---|---|
| `mobile/app/results.tsx` | ~150 | ✓ Modular (separate animated components) |
| `mobile/app/details.tsx` | ~130 | ✓ Modular (SkeletonLoader extracted) |
| `mobile/app/history.tsx` | ~140 | ✓ Modular (Swipeable + SkeletonLoader) |
...
Shimmer animation provides visual feedback while Details screen loads nutrition breakdown. Replaces spinner for modern, progressive disclosure pattern.
...
- [ ] Advanced filters on History (date range, calorie range)
